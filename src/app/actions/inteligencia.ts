...
export async function getHistoricoPrecos(program: string, days: number) {
  ...
  const { data, error } = await supabase
    .from('market_prices')
    .select('*')
    .eq('programa', program.toUpperCase())
...
export async function getAlertasConfig() {
  ...
  const { data, error } = await supabase.from('alertas_config').select('*').eq('user_id', user.id)
  if (error) throw error
  return data || []
}

export async function createAlertaConfig(data: {
  type: string
